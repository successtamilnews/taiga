...
class LoggingService {
  ...
  Future<void> initialize({
    ...
    log(LogLevel.info, 'Logging service initialized for Delivery App', tag: 'LoggingService', metadata: {
  ...
  // Delivery-specific logging methods
  void logDeliveryAssignment({
    ...
    info(
      'Delivery Assignment: $action for delivery $deliveryId',
      tag: 'DeliveryAssignment',
      metadata: {
        'action': action,
  ...
  void logLocationUpdate({
    ...
    debug(
      'Location Update: ${latitude.toStringAsFixed(6)}, ${longitude.toStringAsFixed(6)}',
  ...
  void logRouteNavigation({
    required String action, // 'started', 'deviated', 'arrived'
    required String deliveryId,
    String? routeInfo,
    ...
    info(
      'Route Navigation: $action for delivery $deliveryId',
      tag: 'Navigation',
      metadata: {
        'action': action,
        'delivery_id': deliveryId,
  ...
  void logDeliveryCompletion({
    ...
    info(
      'Delivery Completed: $deliveryId in ${duration.toStringAsFixed(1)} minutes',
      tag: 'DeliveryCompletion',
      metadata: {
        'delivery_id': deliveryId,
  ...
  void logTrafficIncident({
    ...
    warning(
      'Traffic Incident: $incidentType at ${latitude.toStringAsFixed(6)}, ${longitude.toStringAsFixed(6)}',
      tag: 'TrafficIncident',
      metadata: {
  ...
  void logDeliveryIssue({
    ...
    final level = severity == 'high' ? LogLevel.error : LogLevel.warning;
    
    log(
      level,
      'Delivery Issue: $issueType for delivery $deliveryId',
      tag: 'DeliveryIssue',
      metadata: {
  ...
  void logVehicleInfo({
    required String action, // 'updated', 'maintenance_required', 'fuel_low'
    String? vehicleType,
    String? vehiclePlate,
    double? fuelLevel,
    double? mileage,
  ...
  void logWorkingSession({
    required String action, // 'started', 'ended', 'break'
    DateTime? timestamp,
    ...
    info(
      ...
      metadata: {
        'action': action,
        'session_timestamp': timestamp?.toIso8601String(),
  ...
  void logEmergencyEvent({
    ...
    critical(
      'EMERGENCY: $emergencyType at ${latitude.toStringAsFixed(6)}, ${longitude.toStringAsFixed(6)}',
      tag: 'Emergency',
      metadata: {
  ...
  void logPerformanceMetrics({
    ...
    info(
      'Performance Metrics: $metricType',
  ...
  // Get delivery-specific logs
  List<LogEntry> getDeliveryLogs(String deliveryId) {
    return _logBuffer.where((entry) =>
        entry.metadata?['delivery_id'] == deliveryId).toList();
  }

  List<LogEntry> getLocationLogs() {
    return getLogsByTag('LocationTracking');
  }

  List<LogEntry> getEmergencyLogs() {
    return getLogsByTag('Emergency');
  }

  List<LogEntry> getPerformanceLogs() {
    return getLogsByTag('PerformanceMetrics');
  }
  ...
  Future<String> exportLogs({
    ...
    final buffer = StringBuffer();
    buffer.writeln('=== Delivery App Logs Export ===');
  ...
  Future<File> exportLogsToFile({
    ...
    final file = File('${directory.path}/${fileName ?? 'delivery_logs_export_${DateTime.now().millisecondsSinceEpoch}.txt'}');
  ...
  // Console logging with color coding for delivery app
  void _logToConsole(LogEntry entry) {
    String colorCode;
    switch (entry.level) {
      case LogLevel.debug:
        colorCode = '\x1B[96m'; // Light Cyan
        break;
      case LogLevel.info:
        colorCode = '\x1B[94m'; // Light Blue
        break;
      case LogLevel.warning:
        colorCode = '\x1B[93m'; // Light Yellow
        break;
      case LogLevel.error:
        colorCode = '\x1B[91m'; // Light Red
        break;
      case LogLevel.critical:
        colorCode = '\x1B[95m'; // Light Magenta
        break;
    }
    
    debugPrint('$colorCode[DELIVERY] ${entry.toString()}\x1B[0m');
  }
  ...
  Future<void> _setupFileLogging() async {
    try {
      final directory = await getApplicationDocumentsDirectory();
      final logsDir = Directory('${directory.path}/logs/delivery');
      if (!await logsDir.exists()) {
        await logsDir.create(recursive: true);
      }
      
      _logFile = File('${logsDir.path}/delivery_app_${DateTime.now().toIso8601String().split('T')[0]}.log');
  ...
  void _checkAndRotateFile() {
    try {
      if (_logFile?.lengthSync() ?? 0 > _maxFileSize) {
        ...
        final newFileName = 'delivery_app_${tomorrow.toIso8601String().split('T')[0]}.log';
  ...
  void _setupCrashReporting() {
    FlutterError.onError = (FlutterErrorDetails details) {
      critical(
        'Flutter Error in Delivery App: ${details.exception}',
        tag: 'CrashReporter',
        metadata: {
