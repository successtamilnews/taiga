...
class LoggingService {
  ...
  Future<void> initialize({
    ...
    log(LogLevel.info, 'Logging service initialized', tag: 'LoggingService', metadata: {
  ...
  Future<File> exportLogsToFile({
    ...
    final file = File('${directory.path}/${fileName ?? 'logs_export_${DateTime.now().millisecondsSinceEpoch}.txt'}');
  ...
  Future<void> _setupFileLogging() async {
    try {
      ...
      _logFile = File('${logsDir.path}/app_${DateTime.now().toIso8601String().split('T')[0]}.log');
  ...
  void _checkAndRotateFile() {
    try {
      if (_logFile?.lengthSync() ?? 0 > _maxFileSize) {
        // Start new log file tomorrow
        final tomorrow = DateTime.now().add(const Duration(days: 1));
        final newFileName = 'app_${tomorrow.toIso8601String().split('T')[0]}.log';
  ...
  void _scheduleRemoteUpload() {
    // Implement immediate upload for critical logs
    if (_enableRemoteLogging) {
